...
//         }
//       } catch (err) {
//         console.error('Error during fetch:', err); // Log any error that occurs during the fetch
...
const Login = () => {
  ...
  const handleGoogleClick = async () => {
    ...
    try {
      ...
      fetch('http://localhost:8080/auth/google', {
        ...
        .then((data) => {
          console.log(data);
          setLoggedInUsername(data.email); // Update with the email received
        })
        .catch((error) => console.error('Error: ', error));
    } catch (error) {
      console.log(error);
  ...
  const handleLogout = () => {
    fetch('http://localhost:8080/logout', { method: 'POST' })
      .then(response => {
        if (response.ok) {
          setLoggedInUsername(''); // Clear the username on logout
        } else {
